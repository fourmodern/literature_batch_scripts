#!/usr/bin/env python
"""
Safe Parallel Text-Only RAG Builder
생산자-소비자 패턴으로 안전한 병렬 처리 (텍스트만)
"""
...
class SafeTextRAGBuilder:
    ...
    def _write_text_batch(self, collection, batch: List[ProcessedChunk]):
        ...
        try:
            ...
            print(f"✅ Wrote batch of {len(batch)} text chunks")
    ...
    def progress_monitor(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                print(f"\r📊 Progress: {text_count}/{total} papers | Queue: {self.result_queue.qsize()}", end="")
    ...
    def build_databases(self, papers: List[Dict]):
        ...
        try:
            print(f"\n🚀 Starting safe parallel text RAG building with {self.num_workers} workers")
            ...
            print(f"\n✅ Safe parallel text RAG building completed!")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Safe parallel text-only RAG builder")
    ...
    print(f"""
{'='*70}
🔧 Safe Parallel Text RAG Builder
{'='*70}
Workers: {args.workers}
Batch size: {args.batch_size}
Architecture: Producer-Consumer Pattern (Text Only)
...
{'='*70}
    ...
    print(f"""
{'='*70}
✨ Safe Text RAG Building Complete!
{'='*70}
