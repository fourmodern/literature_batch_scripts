...
PLIST_TEMPLATE="$PROJECT_ROOT/config/com.local.zotero-sync.plist"
PLIST_OUTPUT="$PROJECT_ROOT/config/com.local.zotero-sync.plist.configured"
...
echo "   cp $PLIST_OUTPUT ~/Library/LaunchAgents/com.local.zotero-sync.plist"
echo ""
echo "3. 권한 설정:"
echo "   chmod 644 ~/Library/LaunchAgents/com.local.zotero-sync.plist"
echo ""
echo "4. launchd 등록:"
echo "   launchctl load ~/Library/LaunchAgents/com.local.zotero-sync.plist"
echo ""
echo "5. 상태 확인:"
...
if [[ "$response" =~ ^[Yy]$ ]]; then
    ...
    # plist 복사
    cp "$PLIST_OUTPUT" ~/Library/LaunchAgents/com.local.zotero-sync.plist
    echo "✅ plist 파일 복사"
    ...
    echo ""
    echo "테스트 실행:"
    echo "   launchctl start com.local.zotero-sync"
    echo ""
    echo "로그 확인:"
